...
import { cn, formatPriceInCents } from "../../lib/utils";
...
const VERTICAL_LINE_CLASSES =
  "absolute inset-y-0 h-full stroke-2 text-stone-300 transition-colors group-hover:text-stone-400";
...
export async function SkateboardProduct({ id }: Props) {
  ...
  const price = isFilled.number(product.data.price)
    ? formatPriceInCents(product.data.price)
    : "Price Not Available";
  ...
  return (
    <div className="group relative mx-auto w-full max-w-72 px-8 pt-4">
      ...
      <VerticalLine className={cn(VERTICAL_LINE_CLASSES, "left-4")} />
      <VerticalLine className={cn(VERTICAL_LINE_CLASSES, "right-4")} />
      ...
      <div className="-mb-1 flex items-center justify-center overflow-hidden py-4">
        <Scribble
        ...
        <PrismicNextImage
          ...
          width={150}
          className="w-[58%] origin-top transform-gpu transition-transform duration-500 ease-in-out group-hover:scale-150"
        />
      </div>
